Updated LaTeX algorithm block matching the current corner-halving cross-validation driver in `VAEQL_plus.step2_beta_C_tuning.fine_tuner`:
...
\begin{algorithmic}[10]
  ...
  $\beta_{\min}, \beta_{\max}, C_{\min}, C_{\max}$, granularity $\alpha$ \\ \Comment{Default value $\alpha_0=0.01$} \\
