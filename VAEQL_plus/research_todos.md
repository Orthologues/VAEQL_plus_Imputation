...
b. Formalize and, if needed, restructure the existing halving search for $(\beta, C)$ in `VAEQL_plus/step2_beta_C_tuning` as the seamless pre-training fine-tuning stage between Step 0 (Ministral-8B/14B-empowered feature-type assignment), Step 2 (type-aware preprocessing), and Step 3 ($\beta$-DVAE and Q-learning training). Before requesting a code restructure, provide a clear LaTeX pseudo-algorithm block in `VAEQL_plus/step2_beta_C_tuning/codex_pseudo_code.tex` that defines the inputs, corner candidates, cross-validation score, epoch budgets, convergence rule, checkpoint output, and failure behavior. Use that pseudo-algorithm as the implementation contract for the refactor rather than relying on an informal description. <b>(TODO)</b>

c. Complete and validate the Step 0 scaffold under
`VAEQL_plus/step0_SLM_metadata_profiling` as an AWS Batch GPU job. The default
SLM is `mistralai/Ministral-8B-Instruct-2410`. The job definition must request
the required GPU resources. The job reads the raw PDS CSV/JSON input and an
explicit allowlist of approved non-raw metadata files from SSE-C-protected S3
using the Base64-encoded 32-byte customer key. It shall build a sanitized Pandas metadata summary of the raw table and combine it with the approved non-raw metadata bundle for SLM profiling. The SLM shall return feature profiles, and the job shall write a versioned manifest containing the required mappings, missing-value codes, evidence, and, where available, provenance URIs or DOIs.
