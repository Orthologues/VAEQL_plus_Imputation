 ...
 *     --config /abs/path/to/CV_beta_C_tuning_params.json \
 *     --python python3 \
 ...
 * and output targets (results_path, model_outdir). This workflow simply wraps
	 * `python -m VAEQL_plus.step2_beta_C_tuning.fine_tuner --config <json>`
	 * and copies the key artifacts into
 * the publish directory for convenience.
 */

params.config      = "${projectDir}/VAEQL_plus/conf/CV_beta_C_tuning_params.json"
...
process RUN_BETA_C_HALVING {
    ...
    cd ${projectDir}
    ${params.python} -m VAEQL_plus.step2_beta_C_tuning.fine_tuner --config "\${CONFIG_PATH}"
